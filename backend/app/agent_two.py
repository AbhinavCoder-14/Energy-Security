...
def calculate_disruption_impact(risk_payload: GeopoliticalRiskPayload) -> DisruptionImpactPayload:
    ...
    # Scarcity premium: (V_disrupted / V_global) * elasticity
    elasticity_factor = 120.0
    price_scarcity_delta = (total_affected_import_volume / GLOBAL_SUPPLY_MBPD) * elasticity_factor

    # War risk premium: P_base * (Risk Score * 0.0015)
    ...
    return DisruptionImpactPayload(
        ...
        total_days_of_reserve_cover=round(days_of_reserve_remaining, 1),
        affected_import_volume_mbpd=round(total_affected_import_volume, 2),
        market_metrics=MarketImpact(
            brent_crude_price_usd=round(projected_brent_price, 2),
            price_increase_pct=round(price_inc_pct, 2),
            freight_premium_usd_per_barrel=round(freight_premium, 2),
